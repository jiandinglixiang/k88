<template>
  <div @click="goCurrentLottery" class="home-recommend-lottery">
    <div class="type-img"></div>
    <div class="lottery-balls">
      <span class="bg-red-ball" v-for="(ball,index) in redBalls" :key="index">{{ball}}</span>
      <span class="bg-blue-ball" v-for="(ball,index) in blueBalls" :key="index">{{ball}}</span>
    </div>
