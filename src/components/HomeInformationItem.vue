<template>
  <div @click="goWebPage" class="home-information-item">
    <div class="top"></div>
    <div class="information-item-container">
      ...
      <div class="title">
        {{information.title}}
        <span class="sub-title">{{information.sub_title}}</span>
      </div>
...
export default {
  ...
  computed: {
    description () {
      const des = this.information.description
      const lg = des.length
      return lg > 50 ? des.substr(0, 50) + ' ...' : des
    }
  },
  methods: {
    goWebPage () {
...
<style>
  ...
  .home-information-item .information-item-container .img {
    position: absolute;
    width: 40px;
    height: 40px;
    left: 5px;
