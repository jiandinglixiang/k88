<template>
  <div class="football-confirm padding-bottom-90" v-cloak>
    <div class="head">
      <span @click="changeAccount" class="text-light text-normal text-p-l"
            v-if="login && !rechargeResult.success">更换账号</span>
      <div>订单确认页</div>
    </div>
    <div v-if="!expired">
      ...
      <div class="money-confirm">
        <div class="payment-tip text-default">请在
          <count-down :time="confirm.expire_time" @timeOver="timeOver"></count-down>
          内支付订单,到期未支付订单自动失效.
        </div>
        <div class="row text-sm bg-primary">
          ...
          <div @click="submit" class="col col-20 col-center text-ml text-center right">
            付款
          </div>
        </div>
      </div>
