    ...
    <basket-panel-container title="比分详情">
      <template v-if="scores.guest_score">
        <table>
          ...
          <tr>
            <td class="has-icon text-left" width="33%">
              <img :src="detail.guest_logo" alt="" class="">
              {{detail.guest}}
            </td>
            ...
            <td class="text-primary" width="15%">{{totalScores(scores.guest_score)}}</td>
          </tr>
          <tr>
            <td class="has-icon text-left" width="33%">
              <img :src="detail.home_logo" alt="" class="">
              {{detail.home}}
            </td>
            ...
            <td class="text-primary" width="15%">{{totalScores(scores.home_score)}}</td>
          </tr>
          </tbody>
        </table>
      </template>
      ...
      <template v-if="techs.list">
        ...
        <table>
          ...
          <tr v-for="item in techs.list">
            ...
            <td width="27%">
              <total-percentage :percent="item.home_rate" direction="right"></total-percentage>
            </td>
    ...
    <basket-panel-container title="球员数据">
      ...
      <template v-if="players.list">
        <div class="margin-top row text-center has-con">
          <div :class="{'active': active === 'guest'}" @click="tabItemClick('guest', detail.guest_id)"
               class="col has-icon">
            <span><img :src="detail.guest_logo" alt="">{{detail.guest}}</span>
          </div>
          <div :class="{'active': active === 'home'}" @click="tabItemClick('home', detail.home_id)"
               class="col has-icon">
