<template>
  <div class="football-information">
    <information-header :active="activeTabItem" :tabItem="tabItem" @tabClick="tabClickChange"
                        type="football"></information-header>
    <div class="tab-container">
      <div class="tab-container-wrap">
        <div :class="{'show': tabItem[0].key === activeTabItem.key}" class="tab-container-item">
          <template v-if="information.detail.match_status > 1">
            <situation-panel></situation-panel>
          </template>
          <template v-else>
            <div class="block">
              <img alt="暂无比赛数据" class="no-situation" src="../../assets/information/no_situation.png">
              <p class="text-center text-light">暂无比赛数据</p>
            </div>
          </template>
        </div>
        <div :class="{'show': tabItem[1].key === activeTabItem.key}" class="tab-container-item">
          <record-table :data="historyRecord" title="历史交锋"></record-table>
          <record-table :data="information.latestRecord" class="margin-top-10" title="近期战绩"></record-table>
          <record-table :data="information.futureRecord" :isFuture="true" class="margin-top-10"
                        title="未来赛事"></record-table>
        </div>
        <div :class="{'show': tabItem[2].key === activeTabItem.key}" class="tab-container-item">
          <integral-panel></integral-panel>
        </div>
        <div :class="{'show': tabItem[3].key === activeTabItem.key}" class="tab-container-item">
          <plate-panel :data="information.odds[0]" :type="0"></plate-panel>
        </div>
        <div :class="{'show': tabItem[4].key === activeTabItem.key}" class="tab-container-item">
...
import {
  INFORMATION_GAME_POINT,
  INFORMATION_LATEST_ODDS,
  INFORMATION_RECORD_FUTURE_GUEST,
  INFORMATION_RECORD_FUTURE_HOME,
  INFORMATION_RECORD_HISTORY,
  INFORMATION_RECORD_LATEST_GUEST,
  INFORMATION_RECORD_LATEST_HOME,
  INFORMATION_SCHEDULE_DETAIL,
  INFORMATION_SCHEDULE_EVENT,
  INFORMATION_SET_THIRD_ID,
  INFORMATION_TECHNOLOGY_TOTAL
} from '../../store/information/types'
