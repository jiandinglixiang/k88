<template>
  <div class="sports-optimize">
    ...
    <div class="operate-wrap">
      <div>
        <a :class="{selected: operateIndex === 0}" @click="switchOperate(0)" href="javascript:">平均优化</a>
        <a :class="{selected: operateIndex === 1}" @click="switchOperate(1)" href="javascript:">风险最小</a>
        <a :class="{selected: operateIndex === 2}" @click="switchOperate(2)" href="javascript:">奖金最高</a>
      </div>
    </div>
    <div class="list" ref="list">
      ...
      <optimize-item :index="key" :item="item" :key="key" v-for="(item, key) in list"></optimize-item>
    </div>
    <div class="bottom-fixed">
      ...
      <div class="margin-top-10">
        <a @click="payment" class="btn btn-block" href="javascript:">付款</a>
      </div>
    </div>
  </div>
</template>
...
import {
  ...
  SPORTS_OPTIMIZE_INPUT_VALUE,
  SPORTS_OPTIMIZE_ITEM_MONEY,
  SPORTS_OPTIMIZE_REDISTRIBUTION,
  SPORTS_OPTIMIZE_STAKE_DISTRIBUTION
      ...
      this.$store.dispatch(SPORTS_CONFIRM_PAYMENT, result).then(() => {
        if (this.confirm.id) {
          if (this.mine.balance < (this.optimize.count * 2)) {
            ...
            this.$router.push({
              name: 'Payment',
              query: { lack: (this.optimize.count * 2 - this.mine.balance).toFixed(2) }
            })
          } else {
            this.$router.push({
              name: 'PaymentOrder',
