...
<!--      <service-agreement></service-agreement>-->
    ...
    <div class="bottom-fixed">
      <div class="row top">
        <div class="col">
          <custom-select-box :value="confirm.multiple" @change="multipleChange" label="倍数"></custom-select-box>
        </div>
      ...
      this.$store.dispatch(SFC_CONFIRM_PAYMENT, result).then(() => {
        if (this.confirm.id) {
          if (this.mine.balance < (this.confirm.multiple * this.confirm.stakeCount * 2)) {
            ...
            this.$router.push({
              name: 'Payment',
              query: { lack: (this.confirm.multiple * this.confirm.stakeCount * 2 - this.mine.balance).toFixed(2) }
            })
          } else {
            this.$router.push({
              name: 'PaymentOrder',
