<template>
  <div :class="{'k3': isK3()}" class="digital-confirm">
    <v-head :title="current.lottery_name"></v-head>
    <div class="digital-confirm-container">
      <div class="operate-wrap">
        <a @click="increaseNum" href="javascript:"><span class="plus-icon"></span>自选号码</a>
        <a @click="randomTicket" href="javascript:"><span class="plus-icon"></span>机选一注</a>
    ...
    <div class="bottom-fixed">
      <div class="row top">
        <div class="col">
          <custom-select-box :value="confirm.followTimes" @change="followTimesChange" label="追号"></custom-select-box>
        </div>
        <div class="col">
          <custom-select-box :value="confirm.multiple" @change="multipleChange" class="pull-right"
      ...
      <div class="summary">
        <div class="text">
          ...
          <span v-if="current.addTo"><input class="add-to" id="addTo" type="checkbox" v-model="addTo"><label
            for="addTo">追加投注</label></span>
...
import {
  CONFIRM_CLEAR_TICKETS,
  CONFIRM_DELETE_TICKET,
  CONFIRM_RANDOM_TICKET,
  DIGITAL_CONFIRM_PAYMENT,
  FOLLOW_TIMES_CHANGE,
  MULTIPLE_CHANGE
      ...
      this.$store.dispatch(DIGITAL_CONFIRM_PAYMENT, result).then(() => {
        if (this.confirm.id) {
          if (this.mine.balance < (this.stakeCount * this.confirm.followTimes * this.confirm.multiple * (this.addTo ? 3 : 2))) {
            ...
            this.$router.push({
              name: 'Payment',
              query: { lack: (this.stakeCount * this.confirm.followTimes * this.confirm.multiple * (this.addTo ? 3 : 2) - this.mine.balance).toFixed(2) }
            })
          } else {
            this.$router.push({
              name: 'PaymentOrder',
